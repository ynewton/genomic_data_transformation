...
#YN version1
argspec <- c("extract_matrix_rows.r
Usage:
    extract_matrix_rows.r cmap.tab output.tab rows.tab include_flag
Example:
	Rscript extract_matrix_rows.r cmap.tab output_matrix.tab rows.tab TRUE
Options:
	input matrix
	output file name
...
main <- function(argv) {  
	...
	if(include_flag == TRUE){
		data_matrix <- data_matrix[rownames(data_matrix) %in% druglist,]
	}else{
		data_matrix <- data_matrix[!(rownames(data_matrix) %in% druglist),]
	}
